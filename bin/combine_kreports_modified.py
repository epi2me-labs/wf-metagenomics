...
#   --display-headers.........includes header lines mapping samples to
#                             abbreviated names
#                             [default:true]
#   --no-headers..............do not include header lines [default:false]
#   --sample-names............sample names for each kraken report
#                             (separated by spaces)
#                             [if none are given,
#                              each sample is given names S1, S2, etc]
...
# including only the taxonomy levels and
# genomes identified in the Kraken report
class Tree(object):
    """Tree node."""

    def __init__(
        self, name, taxid, level_num,
        level_id, all_reads, lvl_reads,
        children=None, parent=None): # noqa
        """Tree node."""
        self.name = name
        self.taxid = taxid
...
####################################################################
# process_kraken_report
# usage: parses a single line in the kraken
# report and extracts relevant information
...
def main():
    ...
    if len(sample_names) > 0 and len(sample_names) != num_samples:
        sys.stderr.write(
            "Number of sample names provided does"
            "not match number of reports\n")
        sys.exit(1)
    # Map names
    id2names = {}
    id2files = {}
    ...
    sys.stdout.write(
        "\t%i/%i samples processed" % (count_samples, num_samples))
    sys.stdout.flush()
    for r_file in args.r_files:
        count_samples += 1
        sys.stdout.write(
            "\r\t%i/%i samples processed" % (count_samples, num_samples))
        sys.stdout.flush()
        ...
        for line in curr_file:
            ...
            if len(report_vals) < 5:
                continue
            [name, taxid, level_num, level_id, all_reads, level_reads] = report_vals # noqa
            ...
            # Tree Root
            if taxid == '1':
                if count_samples == 1 or root_node == -1:
            ...
            if taxid in taxid2node:
                taxid2node[taxid].add_reads(
                    count_samples, all_reads, level_reads)
            ...
            # Add node to tree
            curr_node = Tree(
                name, taxid, level_num, level_id, 0, 0, None, prev_node)
    ...
    if total_reads[0] == 0:
        o_file.write("%0.4f\t" % (float(u_reads[0])/float(1)*100))
    else:
    ...
    while len(all_nodes) > 0:
        ...
        o_file.write("%0.4f\t" % (float(curr_node.tot_all)/float(total_reads[0])*100)) # noqa
