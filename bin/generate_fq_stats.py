...
parser.add_argument(
    "-f", type=Path, help="Path to master centrifuge table with label", default=None,
)
...
def main(argv=None):
    ...
    master = pd.DataFrame(process_record(fq_path), columns=["readID", "len", "meanqscore"])
    if master_path:
